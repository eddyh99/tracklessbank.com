...
class Swap extends CI_Controller
{
    ...
    public function swapcalculate()
    {
        ...
        if ($amount > 0) {
            ...
            $result = apitrackless("https://api.tracklessbank.com/v1/trackless/swap/swaptrackless_summary", json_encode($mdata));
            if (@$result->code != 200) {
                header("HTTP/1.1 500 Internal Server Error");
                $error = array(
                    "token"     => $this->security->get_csrf_hash(),
    ...
    public function notif()
    {
        ...
        if ($amount > 0) {
            ...
            $result = apitrackless("https://api.tracklessbank.com/v1/trackless/swap/swaptracklessProcess", json_encode($mdata));
            if (@$result->code != 200) {
